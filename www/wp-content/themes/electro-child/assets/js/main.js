...
var _swiperMin = __webpack_require__(14);
...
function onReady() {
  ...
  //   $('.page-template-submit-form').find('.wpcf7-form').fadeOut(400, function() {
  //     $(".contact-form").addClass("form-success");
  //   });
  // }, false );
  document.addEventListener('wpcf7mailsent', function (event) {
    (0, _jquery2.default)(".contact-form").addClass("form-success");
  }, false);

  var swiper = new _swiperMin2.default('.company-slider', {
    speed: 600,
    spaceBetween: 0,
    slidesPerView: 'auto',
    scrollbar: {
  ...
  var newsSwiper = new _swiperMin2.default('.news-slider', {
    speed: 600,
    spaceBetween: 0,
    slidesPerView: 'auto',
    slidesPerGroup: 4,
  ...
  var teamSwiper = window.innerWidth <= 768 ? new _swiperMin2.default('.team-slider', {
    speed: 600,
    spaceBetween: 0,
    slidesPerView: 'auto',
...
var _career = __webpack_require__(17);
...
/**
 * Function that is fired when an event is dispateched and contains a detail element with the node
 * that contains the new HTML.
 *
 * @param CustomEvent event The custom event with the details.
*/

/**
 * Function that is used to search the new share buttonson the dom and attach the default behavior.
 *
 * @param jQuery $source A jQuery Object.
*/
module.exports = function () {
  new _header2.default();
  new _career2.default((0, _jquery2.default)("#career"));

  (0, _jquery2.default)('.wpcf7-file-wrapper').each(function (index, el) {
    new _file2.default(el);
  });
  ...
  if (str1.indexOf(str2) != -1) {
    (0, _jquery2.default)('.cookies-box').hide();
  }

  (0, _jquery2.default)('.cookies-box').find('.got-it-btn').on('click', function (event) {
    event.preventDefault();
    setCookies('acceptCookies', 10, 365);
    (0, _jquery2.default)(this).closest('.cookies-box').fadeOut();
    /* Act on the event */
  });
  ...
  (function ($) {
    $(function () {
      ...
      $('.policy').on('shown.bs.modal', function () {
        // $('#myInput').trigger('focus')
        // var newURL = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname + $(this).attr('id');
        // console.log(newURL);
        // var currentState = window.history.state;
        if (!hash) {
          if (!$(hash).length) {
            window.history.pushState("", "", $(this)[0].baseURI + "#" + $(this).attr('id'));
          }
        }
      ...
      $('.policy').on('hidden.bs.modal', function () {
        ...
        // window.history.pushState("", "", $(this)[0].baseURI.replace(window.location.hash, ''));
        // window.history.pushState("", "", $(this)[0].baseURI + "#" + $(this).attr('id'));

        // window.location.replace('#' + newURL);
      });
      if (hash == '#terms-of-use' || hash == '#cookie-policy' || hash == '#privacy-policy') {
        if ($(hash).length) {
          $(hash).modal('show');
        }
      }
...
var Header = function () {
    ...
    function Header() {
        ...
        this.$mainOverlay = (0, _jquery2.default)('.main-overlay');
        this.$hamburger = (0, _jquery2.default)('#menu-btn');
    ...
    _createClass(Header, [{
        ...
        value: function onScroll() {
            ...
            obj.mainNavLinks.each(function () {
                var currLink = (0, _jquery2.default)(this);
                var refElement = (0, _jquery2.default)(currLink.attr("href"));
                ...
                } else if ((0, _jquery2.default)(window).scrollTop() + (0, _jquery2.default)(window).height() == (0, _jquery2.default)(document).height()) {
                    obj.mainNavLinks.removeClass("active");
        ...
        value: function events() {
            ...
            this.mainNavLinks.on("click", function (event) {
                var $this = (0, _jquery2.default)(this),
                    href = $this.attr("href"),
                    topY = (0, _jquery2.default)(href).offset().top,
                    mainMenuHeight = window.innerWidth > 1024 ? obj.mainNav.height() + 40 : 60,
                    sp = Math.abs(window.scrollY - topY) / 2;

                (0, _jquery2.default)('html, body').stop().animate({
                    scrollTop: topY - mainMenuHeight
                }, sp);
                obj.toggleMenu();
                event.preventDefault();
                return false;
        ...
        /**
         * Function used to update the UI based on the scroll class.
         *
         * @param {number} percentage - Number from [0 - 100].
         */
...
var Career = function () {
  ...
  */
	function Career(el) {
		...
		this.element = el;
		this.siteUrl = (0, _jquery2.default)('head').find('link[rel="baseurl"]').attr('href');
		this.apiToken = (0, _jquery2.default)('head').find('link[rel="api-token"]').attr('href');
		this.careerData = [];

		var recruitingModal = (0, _jquery2.default)("#recruiting-modal");

		this.modal = {
			title: recruitingModal.find(".modal-title"),
			company: recruitingModal.find(".md-company"),
			titleDesk: recruitingModal.find(".md-title-desk"),
			content: recruitingModal.find(".modal-content"),
			emailBtn: recruitingModal.find(".email-btn"),
			emailBtnHref: recruitingModal.find(".email-btn").attr("href")
		};

		// $.get(this.siteUrl + "/wp-json/acf/v3/pages/366/recruiting_item", ( data ) => {
		/*_jquery2.default.get(this.siteUrl + "/wp-json/acf/v3/pages/100/recruiting_item", function (data) {
			_this.careerData = data.recruiting_item;
		});*/

		_jquery2.default.get(this.siteUrl + "/wp-api/" + this.apiToken + "/recruiting_item", function (data) {
			_this.careerData = data.recruiting_item;
		});
	...
	_createClass(Career, [{
		key: 'generateContent',
		value: function generateContent(data) {}
	}, {
		key: 'events',
		value: function events() {
			var _this2 = this;

			this.element.find(".details-btn").on('click', function (event) {
				var currentItemData = _this2.careerData[(0, _jquery2.default)(event.currentTarget).attr("data-index")];
				var titleDeskDataWorkTime = currentItemData.work_time ? '<strong class="title-desk-item">' + currentItemData.work_time + '</strong>' : '';
				var titleDeskDataWorkLocation = currentItemData.work_location ? '<strong class="title-desk-item">' + currentItemData.work_location + '</strong>' : '';
				var titleDeskSalary = currentItemData.salary ? '<strong class="title-desk-item">' + currentItemData.salary + '</strong>' : '';
				var titleDeskData = titleDeskDataWorkTime + titleDeskDataWorkLocation + titleDeskSalary;

				_this2.modal.title.text(currentItemData.position_title);
				_this2.modal.titleDesk.html(titleDeskData);
				_this2.modal.content.html(currentItemData.content);
				_this2.modal.company.html(currentItemData.company_name);
				_this2.modal.emailBtn.attr("href", _this2.modal.emailBtnHref + currentItemData.position_title + " | " + currentItemData.work_time + " | " + currentItemData.work_location);

				// if (currentItemData.company_name.length > 0) {
				// 	this.modal.company.hide();
				// }
			});
		}
	}]);
...
var File = function () {
	function File(el) {
		...
		this.$fileName = this.$el.find('.wpcf7-file-name');
		this.$fileLabel = this.$el.find('.wpcf7-file-label');

		this.inputStyle = this.$el.hasClass('input-style');
	...
	_createClass(File, [{
		...
		value: function events() {
			...
			this.$input.on('change', function (event) {
				if (!_this.inputStyle) {
					...
					_this.updateFileButton();
				}
			});

			this.$el.on('click', '[data-remove-file]', function (event) {
				event.preventDefault();

				_this.$input.val('');
				_this.$el.removeClass('file-selected');
		...
		key: 'updateFileButton',
		value: function updateFileButton() {
			if (this.$input[0].files.length > 0) {
				this.$fileName.find('i').text(this.$input[0].files[0].name);
		...
		value: function updateFileList() {
			...
			if (this.$input[0].files.length > 0) {
				...
				for (var i = 0; i < this.$input[0].files.length; i++) {
					files += '<li><span data-remove-file="' + i + '">x</span>' + this.$input[0].files[i].name + '</li>';
				}
...
module.exports = function () {
  new _modals2.default((0, _jquery2.default)(".base-modal"));
};
...
function Modal(el) {
  this.md = el;
  this.md.append('<div id="md-overlay" class="main-overlay"></div>');
  this.$pbody = (0, _jquery2.default)("body");
  this.$mainNav = (0, _jquery2.default)("#main-nav");
  this.initEvents();
}
...
Modal.prototype = {
  initEvents: function initEvents() {
    ...
    _this.md.find(".close-btn").on('click', function (e) {
      _this.closeModal();
    });

    (0, _jquery2.default)("#md-overlay").on('click', function () {
      _this.closeModal();
    });

    (0, _jquery2.default)(".modal-btn").on('click', function (event) {
      event.stopPropagation();
      event.preventDefault();
      var _thisMd = (0, _jquery2.default)(this);

      _this.$pbody.addClass("modal-open");
      _this.$pbody.css({
      ...
      (0, _jquery2.default)(_thisMd.attr("data-id")).addClass("open");
      (0, _jquery2.default)("#md-overlay").fadeIn(100, function () {
        (0, _jquery2.default)(_thisMd.attr("data-id")).addClass("animate");
      });
    });
  },
  closeModal: function closeModal() {
    var _this = this;
    _this.md.removeClass("animate");
    (0, _jquery2.default)("#md-overlay").fadeOut(200, function () {
      _this.md.removeClass("open");

      _this.$pbody.removeClass("modal-open");
      _this.$pbody.css({
        "padding-right": 0
      });
      _this.$mainNav.css({
    ...
    _this.md.find('.wpcf7-form').removeClass('sent');
    _this.md.find('.wpcf7-response-output').removeClass('wpcf7-mail-sent-ok').attr('style', '');

    // _this.md.find("form").find(':input').each(function() {
    //     switch(this.type) {
    //         case 'text':
    //         case 'textarea':
    //         $(this).val('');
...
var _scrollmagic = __webpack_require__(27);
...
var $smAnimationTime = 0.78,
    scene = [];

(0, _jquery2.default)('.sm-animation, .smd-animation').each(function (index, el) {
   var _this = (0, _jquery2.default)(this),
       sm = {};
   sm.triggerHook = _this.attr('data-sm-trigger-hook') || 1;
   sm.offset = _this.attr('data-sm-offset') || 0;
   sm.delay = _this.attr('data-sm-delay') || 0;
   sm.time = _this.attr('data-sm-time') || $smAnimationTime;
   sm.reverse = _this.attr('data-sm-reverse') || false;
   sm.triggerContainer = _this.attr('data-sm-container') || _this[0];

   // console.log(sm.triggerContainer) ;

   // TweenMax.set(_this, { transitionDuration: sm.time+'s', transitionDelay: sm.delay+'s' });
   scene.push(new _scrollmagic2.default.Scene({
      triggerElement: sm.triggerContainer,
      triggerHook: sm.triggerHook,
      offset: sm.offset,
      reverse: sm.reverse

   }).setClassToggle(_this[0], 'animate'));
});

(0, _jquery2.default)('.sm-counting').each(function (index, el) {
   var _this = (0, _jquery2.default)(this),
       sm = {};
   sm.triggerHook = _this.attr('data-sm-trigger-hook') || 1;
   sm.offset = _this.attr('data-sm-offset') || 0;
   ...
   scene.push(new _scrollmagic2.default.Scene({
      ...
      (0, _jquery2.default)(_this[0]).prop('Counter', 0).animate({
         Counter: (0, _jquery2.default)(_this[0]).text()
      }, {
         duration: 2000,
         easing: 'swing',
         step: function step(now) {
            (0, _jquery2.default)(_this[0]).text(Math.ceil(now));
            (0, _jquery2.default)(_this[0]).text((0, _jquery2.default)(_this[0]).text().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
         }
      });
   }));
});
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__TweenMax__ = __webpack_require__(25);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__TweenLite__ = __webpack_require__(0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__TweenMaxBase__ = __webpack_require__(26);
