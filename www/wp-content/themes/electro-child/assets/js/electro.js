...
(function($) {
		...
		if( electro_options.enable_live_search == '1' ) {
			...
			var searchProducts = new Bloodhound({
				...
				remote: {
					url: remote_url,
					wildcard: '%QUERY',
					rateLimitWait: 1000 // Added by Armen, 2019-12-22
				},
			...
			$('.navbar-search .product-search-field, .handheld-header-v2 .woocommerce-product-search .search-field, .mobile-header-v1 .woocommerce-product-search .search-field, .mobile-header-v2 .woocommerce-product-search .search-field')
				.typeahead( electro_options.typeahead_options, {
					name: 'search',
			...
			).on('keyup', function(e){ // Added by Armen, 2020-06-06
				if(e.keyCode == 13){
					// Скрываем выпадающий список пустого результата при нажатии на Enter в поле поиска.
					$(this).parents('.twitter-typeahead').addClass('hide-dropdown');
				}
			});
		}
	...
	if ($(".dokan-store #dokan-content").hasClass("store-page-wrap")) {
		$(".dokan-store-tabs ul li:first-child").addClass('active');
	}

	if ($(".dokan-store #dokan-content").hasClass("store-review-wrap")) {
		$(".dokan-store-tabs ul li:nth-child(2)").addClass('active');
	}
