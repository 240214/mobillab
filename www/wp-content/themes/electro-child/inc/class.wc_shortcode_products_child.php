 ...
 */
class WC_Shortcode_Products_Child{
	...
	protected function get_query_results(){
		...
		if(isset($transient_value['value'], $transient_value['version']) && $transient_value['version'] === $transient_version){
			...
			#\Digidez\Functions::_debug($query);
		...
		#\Digidez\Functions::_debug($results);
		return $results;
	}
	...
	protected function product_loop(){
		...
		if($products && $products->ids){
			$cache_file = 'raw-'.md5(implode('-', $products->ids));
			#Functions::_debug($cache_file); exit;
			$products_html = Functions::get_cache_file($cache_file);

			if(is_null($products_html)){
				ob_start();
				...
				if($products && $products->ids){
					...
					if(wc_get_loop_prop('total')){
						foreach($products->ids as $product_id){
							//$GLOBALS['post'] = Functions::get_post_from_cache($product_id);
				...
				Functions::set_cache_file($cache_file, $products_html);
			}
		}else{
			do_action("woocommerce_shortcode_{$this->type}_loop_no_results", $this->attributes);
		}

		return '<div class="'.esc_attr(implode(' ', $classes)).'">'.$products_html.'</div>';
	}
