...
$products = array();
$_sql = "SELECT m.* FROM mod_products_price_change m WHERE 1=1 $where ORDER BY $sort_order ";
$products = WCPL_Data_Source::get_results($_sql, ARRAY_A);
$products_found = count($products);
$products_ids = $products_ids_auto = $products_prices_auto = array();
...
<div class="bootstrap form-table" data-example-id="hoverable-table">
	...
	<div>
		...
		<?php if(!empty($products)):?>
			<table id="change_products_nacenka" class="table table-hover table-striped izmenit price-diff">
				<thead>
					<tr>
				...
				<?php foreach($products as $key => $val):
					...
					if(is_array($diapasons[$val['id_supplier']])) {
						...
						foreach($diapasons[$val['id_supplier']] as $kk => $vv) {
							if($val['new_price'] >= $vv['price_low'] && $val['new_price'] <= $vv['price_high'] ) {
								...
								$products_ids_auto[] = $val['products_id'];
								$products_prices_auto[] = $val['products_price'];
								$autoMode = true;
								break;
							}
						}
					...
					<tr id="tr_nacenka_<?=$val['products_id'];?>">
						...
						<td id="td_nacenka_<?=$val['products_id'];?>" align="right"><span class="nacenka" style="color: <?=$color1;?>"><?=floor($val['products_price'] - $val['new_price']).( $inputReadonly ? "<br />{$val['nacenka']}" : "");?></span></td>
						<td>
							<div class="btn-group">
								<button type="button" role="button" data-source="change_nacenka" data-action="js_ajax" data-pid="<?=$val['products_id'];?>" class="btn btn-danger btn-sm"><?=esc_attr__('Update price', PINLOADER_TEXT_DOMAIN);?></button>
					...
					<?php endforeach;?>
					<?php if(count($products_ids_auto) == 1){
						$products_ids_auto[] = 0;
						$products_prices_auto[] = 0;
					}?>
				</tbody>
			</table>
			<?php if($manualMode):?>
			<div class="text-center">
				<a href="javascript:;" role="button" class="btn btn-danger btn-lg"
				   data-source="change_all_manual"
				   data-action="js_ajax"
				   data-rowprefix="tr_nacenka_"
				   data-ids="<?=implode(",", $products_ids)?>">Изменить все</a>
			</div>
			<?php endif;?>
			<?php if($autoMode):?>
			<div class="text-center">
				<a href="javascript:;" role="button" class="btn btn-danger btn-lg"
				   data-source="change_all_auto"
				   data-action="js_ajax"
				   data-rowprefix="tr_nacenka_"
				   data-ids="<?=implode(",", $products_ids_auto)?>"
				   data-prices="<?=implode(",", $products_prices_auto)?>">Изменить все автоматом</a>
			</div>
			<?php endif;?>
		<?php endif;?>
